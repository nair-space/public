<script lang="ts">
	...
	const jumlahInfaqUangDb = $derived.by(() =>
		data.infaqList.reduce((total, record) => {
			return total + Number(record.infaqUangIncome || 0);
		}, 0)
	);
	...
	function editRecord(record: typeof data.infaqList[0]) {
		editingId = record.id;
		idNota = record.idNotaInput?.toString() || '';
		nama = record.nama;
		infaqUang = record.infaqUangIncome?.toString() || '';
		window.scrollTo({ top: 0, behavior: 'smooth' });
	}

	async function deleteRecord(record: typeof data.infaqList[0]) {
		...
		try {
			...
			if (result.type === 'success') {
				...
				alert('Gagal menghapus data infaq');
			}
		} catch (e) {
			console.error('Failed to delete infaq:', e);
			alert('Gagal menghapus data infaq');
		}
	}
...
<div class="space-y-6">
	...
	<GlassCard title={editingId ? 'Edit Infaq' : 'Input Infaq'}>
		<form method="POST" action={editingId ? '?/update' : '?/save'} use:enhance class="grid grid-cols-1 gap-4 md:grid-cols-2">
			...
			<GlassInput label="Infaq (Uang / Rp)" name="infaq_uang" type="number" placeholder="0" bind:value={infaqUang} />
	...
	<GlassCard title="Ringkasan Pemasukan">
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
			<div class="flex flex-col gap-1.5">
				<p class="text-sm font-medium text-white/70">Jumlah Pemasukan Infaq (Uang / Rp)</p>
				<div class="glass-input px-4 py-2 text-white/80">
					{formatRupiah(jumlahInfaqUangDb)}
				</div>
				<p class="text-xs text-white/50">Akumulasi seluruh infaq uang</p>
			</div>
		</div>
	</GlassCard>

	<GlassCard title="Riwayat Infaq">
		{#if data.infaqList.length === 0}
			<div class="flex items-center justify-center py-8 text-white/30">
				<p class="text-sm">Belum ada data. Silakan input infaq di atas.</p>
			</div>
		{:else}
				...
				<table class="w-full text-sm">
					<thead>
						<tr class="border-b border-white/10 text-left text-white/50">
							...
							<th class="px-3 py-3 text-right font-medium">Infaq Uang</th>
							<th class="px-3 py-3 font-medium">Tanggal</th>
							<th class="px-3 py-3 text-center font-medium">Aksi</th>
						</tr>
					</thead>
					<tbody>
						{#each data.infaqList as record (record.id)}
							<tr class="border-b border-white/5 transition-colors hover:bg-white/5">
								...
								<td class="px-3 py-3 text-right text-emerald-400">
									{record.infaqUangIncome ? formatRupiah(record.infaqUangIncome) : '-'}
								</td>
